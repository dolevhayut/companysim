...
- Generator version 2 defaults `asOf` to the current UTC day while keeping it explicit in normalized config. It uses namespace SHA-256 IDs, seeded Mulberry32 values, varied lifecycle/activity timestamps, renewal and target-date buckets, skewed child distributions, and deterministic status/value distributions. History has explicit `has_role` intervals and promotion events. Events span people, customers, projects and tickets while retaining `type: event` and using `eventType` to distinguish the transition kind. Version 1 exports remain importable.
...
- Agent eval traces are provider-neutral JSON/YAML with a final answer, tool names/arguments and optional client-reported latency/cost. Scoring is deterministic and inspectable; it checks the applied scenario state, required retrieval calls, exact target-ID citations and an allowlist of the 20 read-only CompanySim MCP tools. It does not invoke a model or claim semantic answer quality.
