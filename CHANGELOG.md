...
- Made Scenario Lab test packs executable. `company scenario validate FILE` checks target identities and captured baseline values; `company scenario run FILE --branch NAME` applies the pack atomically and records the scenario event/history. JSON and YAML are accepted, and stale packs fail before writes.
- Added a provider-independent agent eval runner. `company eval template PACK` creates the portable trace shape and `company eval run PACK TRACE` returns a deterministic scorecard for scenario state, CompanySim tool use, entity-ID citations and read-only compliance while preserving client-supplied latency/cost evidence.
